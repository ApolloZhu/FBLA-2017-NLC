...
extension Localized {
    static let COMMENTS = NSLocalizedString("Comments", comment: "Title on page where displays comments for an item")
    ...
    func send() {
        ...
        if let iid = iid, let message = textField.text?.content, !message.isBlank {
            DispatchQueue.global(qos: .userInteractive).async {
                Comment.saveNew(iid: iid, uid: Account.shared.uid ?? "0", message: message)
            }
            textField.text = nil
        }
    }
